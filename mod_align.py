"""This is a modified version of the Penn Alignment Toolkit align.py file. The 
 original source code can be downloaded here: 
...
def writeTextFile(outfile, words):
    outfile = outfile.split('.')[0] + '_seg.txt'
    fp = open(outfile, 'wb')
    for word_lst in words:
        fp.write(word_lst[0] + '\n')          # word
        for phone,str_t,end_t in word_lst[1:]:
            fp.write(phone+' '+str(str_t)+' '+str(end_t)+'\n') # phone, str_t, end_t
    fp.close()
...
def align_external(args):
    ...
    if os.path.exists("dict.local"):
        ...
        os.system("cat " + mypath + "/cvc_dict > " + word_dictionary)
    ...
    words = readAlignedMLF(output_mlf, SR, float(wave_start))
    writeTextGrid(outfile, words)
    writeTextFile(outfile, words)
